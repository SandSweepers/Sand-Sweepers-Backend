...
exports.findUserAchievements = async (req, res) => {
  const userId = req.params.id;

  try {
    const data = await Achievements.findByPk(userId);
    if (data) {
      res.send(data);
    } else {
      res.status(404).send({
...
exports.signUp = async (req, res) => {
  try {
    ...
    const existingUser = await Achievements.findOne({ where: { username } });
    if (existingUser) {
    ...
    const newUser = await Achievements.create({
      name,
      username,
...
exports.promoteToAdmin = async (req, res) => {
  ...
  try {
    ...
    const user = await Achievements.findByPk(userId);
    if (!user) {
...
exports.delete = async (req, res) => {
  const deleting = await Achievements.destroy({ where: { id: req.params.id } });
...
exports.update = async (req, res) => {
  try {
    ...
    const user = await Achievements.findByPk(id);
    if (!user) {
      return res.status(404).json({ message: "User not found." });
    }

    if (username) {
      const existingUser = await Achievements.findOne({
    ...
    const [updated] = await Achievements.update(updateData, { where: { id } });

    if (updated) {
      const updatedUser = await Achievements.findByPk(id);
